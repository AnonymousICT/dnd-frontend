...
export default function SpecificEquipment() {
  ...
  const { selectCharacter, characterItems, setCharacterItems } = useContext(
    Context
  );
      ...
      {!name ? null : (
        <section className="specific-equipment">
          ...
          {!specificEquipment["2h_damage"] ? null : (
            <p>
              Two-handed damage: {specificEquipment["2h_damage"].damage_dice}{" "}
              {specificEquipment["2h_damage"].damage_type.name}
            </p>
          )}

          {!range ? null : (
            <p>
              {range.normal}ft{!range.long ? null : `/${range.long}ft`}
            </p>
          )}
          ...
          {!selectCharacter ? null : (
            <button onClick={addToCharacter} data-key={uuid()}>
              Add to Character
            </button>
          )}
        </section>
