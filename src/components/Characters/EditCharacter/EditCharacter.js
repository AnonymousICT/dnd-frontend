...
export default function EditCharacter() {
    ...
    return (
        <div className="slider-container">
            <div tabIndex="0" className="character-container" onKeyDown={handleKeyDown} onKeyUp={(e) => e.preventDefault()}>
                <div>
                    <BasicInfo />
                    <BattleStats 
                        userClass={userClass}
                ...
                <Spellbook filteredLevel={filteredLevel} /> : <h2>No Spells Dummy</h2>}
            </div>
            <button className="nav previous" disabled={pager === 0} onClick={handleSlideMove}>➤</button>
            <button className="nav next" disabled={pager === (nonSpellCaster.includes(currentCharacter.job) ? 4 : 5)} 
            onClick={handleSlideMove}>➤</button>
            <div className="page-buttons">
                <button className={`character${pager === 0 ? " active" : ""}`} title="Character Summary" onClick={handleSlideMove}><span role="img" aria-label="character">👤</span></button>
                <button className={`attributes${pager === 1 ? " active" : ""}`} title="Attributes and Saving Throws" onClick={handleSlideMove}><span role="img" aria-label="attributes">💪</span></button>
                <button className={`skills${pager === 2 ? " active" : ""}`} title="Skills" onClick={handleSlideMove}><span role="img" aria-label="skills">🎭</span></button>
                <button className={`languages${pager === 3 ? " active" : ""}`} title="Languages" onClick={handleSlideMove}><span role="img" aria-label="languages">🗣️</span></button>
                <button className={`equipment${pager === 4 ? " active" : ""}`} title="Equipment" onClick={handleSlideMove}><span role="img" aria-label="equipment">⚔️</span></button>
                {!nonSpellCaster.includes(currentCharacter.job) ? 
                    <button className={`spells${pager === 5 ? " active" : ""}`} title="Spells" onClick={handleSlideMove}><span role="img" aria-label="spells">🔮</span></button> 
                    : null}
            </div>
        </div>
    )
